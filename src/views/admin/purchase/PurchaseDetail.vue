    ...
    <div>
        ...
        <Tabs>
          ...
          <TabPane label="订单详情">
            <Form :label-width="80">
              <FormItem label="采购单号:">
                <Input type="text" v-model="orderDetail.puchaseNo" disabled></Input>
              </FormItem>
              <FormItem label="仓库名称:">
                <Input type="text" v-model="orderDetail.wareHouseName" disabled></Input>
              </FormItem>
              <FormItem label="仓库地址:">
                <Input type="text" v-model="orderDetail.wareHouseAddress" disabled></Input>
              </FormItem>
              <FormItem label="门店名称:">
                <Input type="text" v-model="orderDetail.storeName" disabled></Input>
              </FormItem>
              <FormItem label="门店地址:">
                <Input type="text" v-model="orderDetail.storeAddress" disabled></Input>
              </FormItem>
            </Form>
          </TabPane>
        </Tabs>
...
import {
  manageListPurchasePros,
  getPurchaseStoreWarhouseInfo
...
export default {
    ...
    return {
      form: {
        data: {
          ...
          proName: "",
          proBarcode: "",
          procureNum: this.$route.params.procureNo
        },
  ...
  created() {
    let stPchNo = this.form.data.procureNum;
    getPurchaseStoreWarhouseInfo({ stPchNo })
      .then(response => {
        this.orderDetail = Object.assign({}, this.orderDetail, response);
      })
      .catch();
  },
  methods: {
